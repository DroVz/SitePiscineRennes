    ...
    if (!$code == null) {
        ...
        if ($code->getFormule()->getActivite()->getReservation()) {
            echo '<form method="POST" action="index.php?action=verif&step=reservation">
                ...
                <input type="hidden" id="id_activite" name="id_activite" value=' . $code->getFormule()->getActivite()->getId_activite() . ' /input>
                <input type="hidden" id="nb_entrees" name="nb_entrees" value=' . $code->getFormule()->getNb_entrees() . ' /input>
